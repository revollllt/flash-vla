...
Read this after gathering the metrics (`05-analysis-dimensions.md`). Machine numbers are cited by their `hardware-unit-test` tag — read the value and validity range with `python3 .agents/skills/hardware-unit-test/scripts/constants.py --tag <t>`; restating them here would let two copies drift.
