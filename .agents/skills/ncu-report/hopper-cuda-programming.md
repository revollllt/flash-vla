...
Machine numbers are cited by their `hardware-unit-test` tag (`python3 .agents/skills/hardware-unit-test/scripts/constants.py --tag <t>`); this document never restates a measured value. The `kernel-design` skill's sm90 wiki (`references/wiki/README.md`) holds the symptom-indexed optimization experience and its templates show how each mechanism is spelled — this document is the bridge from an NCU signal to those.
