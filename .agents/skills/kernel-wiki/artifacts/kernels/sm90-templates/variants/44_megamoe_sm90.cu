...
//
// FIVE MECHANISMS THAT ARE THE POINT -- two dependent stages over one pool
// without deadlock (`get_num_l1_warmup_waves`, and an L2 task waiting on the
// L1 task COUNTER), a ring of pool blocks with four use-scaled counters
// (`get_num_max_live_pool_blocks` sizes it so the warmup cannot outrun it),
// granularity-8 interleaved gate/up rows (SGLang's `_interleave_l1_weight_only`
// is the offline half), the top-k weight applied in the L1 epilogue, and task
// info as a 2-slot shared-memory pipeline from one scheduler warp to four
// consumers -- are the portable rules on [kernel-megakernel-forms].
//
