...
//
// sm90 has NO sub-8-bit tensor core (`mma_sm90_gmma.hpp` has no e2m1 atom),
// so every INT4 and FP4 kernel on H100 unpacks weights in registers between
// the shared tile and the MMA, as bit arithmetic on the critical path of a
// weight-bandwidth-bound kernel.  The tricks, their origin in Marlin
// (IST-DASLab, Apache-2.0) as carried by vLLM/SGLang and in FasterTransformer,
// and why a Blackwell port deletes this header instead of translating it:
// [technique-register-unpack-sub-byte].
