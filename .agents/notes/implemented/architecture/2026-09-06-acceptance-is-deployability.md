...
3. **An absolute promotion bar, and a validity limit on the run.**
   ...
   top of the control spread. `control_spread_max_ms` (0.05 ms) is the most
   the two control legs may disagree on chunk `min`; above it
   `benchmarks latency` marks the run invalid and the gate blocks. The
   control spread keeps its role as the minimum detectable effect for the
   other statistics.
...
- A run with a control spread above 0.05 ms blocks and is rerun rather than
  read.
