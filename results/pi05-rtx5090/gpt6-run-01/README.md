...
- Profiling and the initial floor estimate are recorded below. The latest focused expert trace is after 019 and the latest whole-forward overview is after 023; both are diagnostic rather than end-to-end timing results.
...
## 022 — Rounded vision GELU epilogue, withdrawn after inconclusive deployment timing

The fixed cfg10 candidate adds GELU after the original accumulator-plus-bias BF16 roundtrip, with explicit RN arithmetic around the same tanhf function. Native LayerNorm remains unchanged. The candidate and control share one CUTLASS library; other vision sites retain cfg10 without activation. All 27 real layers match bitwise. The candidate uses 156 registers versus 160 for control, with no spills and 61,440 bytes shared memory. Local complete-chain ABBA is A1.293344/B1.252384/B1.262688/A1.296096 ms, only 0.037184 ms mean gain. The source and resource evidence are documented in lab/pi05/vision_rounded_gelu_screen.md.

Full-depth official comparison passed with action metrics equal to 021, and the scoped CPU Target check passed 8 declarations and 1 route check. Fresh-process deployment ABBA gives A32.067026/B32.046447/B32.037269/A32.181948 ms. Its 0.082629 ms mean-of-medians difference is smaller than the 0.114922 ms control drift; B drift is 0.009178 ms and nearest separation only 0.020579 ms. One fixed reverse BAAB was therefore declared before collecting more data.

The reverse block gives B32.101932/A32.154507/A32.186776/B32.190033 ms. Its mean difference is 0.024659 ms, but the medians overlap; A/B drift is 0.032269/0.088102 ms. The first block ended at SM clocks 2865/2857/2857/2857 MHz and temperatures 59/57/59/57 C; the reverse block ended at 2857/2857/2857/2850 MHz and 59/60/62/60 C. Memory remained 13801 MHz. These ending snapshots cannot isolate the frequency contribution. The result remains inconclusive, so no further repetitions were run.

The reverse block executed all four CLI commands consecutively in one serial shell, each with a fresh worker process, after compilation and official checks were complete. The initial block had manual intervals. Both use the existing 5 warmup/100 measurements and identical workload; their idle intervals are not assumed identical. Future comparison blocks will use this consecutive fresh-process execution to reduce operator-dependent gaps, without changing the measured forward or weight/input/precision policy.

The loaded route maps differ only at vision_encoder_norm_ffn_up. Production source, registry and route were withdrawn in 80818f9; the original candidate remains reviewable at 8d88302, and the experiment source at f46185a includes its lab-only resource query. All eight deployment reports, official comparison, 60 local samples and predeclared decision remain recorded. The curve marks this trial withdrawn and the shipped plan retains 021.

## Stopped 128-thread expert softmax screen

A same-library fixed comparison changes only the softmax CTA from 256 to 128 threads, preserving FP32 scale, runtime mask, expf, reciprocal and BF16 stores. The different summation order changes 18 probability elements and 7 output elements across nine actual cases, but all existing shallow tolerances pass. The complete reset-inclusive nine-call ABBA is A109.940/B109.878/B109.918/A110.040 us. Mean gain is only 0.092 us per nine calls, below A drift 0.100 us; B drift is 0.040 us. This provides insufficient local evidence to advance to deployment. Registers/shared memory are 39/32 B for 128 threads versus 32/64 B for the original, with no spills. No other thread count was tested and production was unchanged. The exact native patch, all 60 samples and numerical/resource evidence are in lab/sm120/pi05_attention_softmax_128_probe.md and results/rtx5090-pi05/gpt6-attention-softmax-128.

## Rejected M16 CUTLASS expert FFN down

A different fixed down experiment retains Stream-K, eight stages and the existing rounded gated epilogue while changing CTA/warp M from 32 to 16. M50 still pads to 64, so no tail-padding improvement was assumed. Three constant cases cover all 50 rows exactly; 180 real calls pass shallow checks (worst rel_rms 0.0001540602504), and three same-mainloop decompositions are exact.

Actual metadata confirms both routes use grid 298 and block 64. The candidate has 80 registers versus 120, 40 KiB shared versus 48 KiB, and workspace 1,393,408 versus 2,622,080 bytes, with no spills. Reset-inclusive ABBA is A10.569600/B13.077156/B13.084445/A10.581867 us/call. The candidate is slower by 2.505067 us/call, far above A/B drift 0.012267/0.007289 us. It is rejected without another tile or deployment run. Smaller resource counts did not establish lower latency, and the experiment does not isolate the cause. Native additions and the experiment binary were removed; lab/pi05/cutlass_m16_down.md links the exact patch, coverage, metadata, all 180 errors and 60 samples. Production still uses the 021 plan.

## Stopped exact-shape prefix cfg0 reuse

Reuse of the existing cfg0 GEMM for prefix QKV M968/K2048/N2560 leaves native RMSNorm/RoPE, BF16 projected storage and all addresses unchanged. All 18 actual-layer Q/K/V/x_norm outputs match bitwise. Complete-chain ABBA across the 180 MiB of real weights gives A1.043584/B1.046688/B1.047520/A1.052352 ms. The apparent mean gain, 0.000864 ms, is below control drift 0.008768 ms; candidate/control ranges overlap. This stops the reuse candidate before production changes or deployment timing. lab/pi05/cutlass_prefix_qkv.md and results/rtx5090-pi05/gpt6-prefix-cfg0/local.json retain all 60 samples and exact checks.

## Failed fixed expert down warp-N layout
...
## Rejected expert softmax/PV fusion

A fixed CuTe kernel keeps QK separate, performs the original FP32 scaled masked softmax, rounds probabilities to BF16, then computes PV with FP32 accumulation. Its 16x32x64 tile repeats each row's full softmax eight times across output-N tiles. It compiles at 166 registers, 47,104 shared bytes and no spills; actual metadata reports 200 CTAs and two active CTAs per SM. All nine real cases pass existing shallow probability/output tolerance, without bitwise output equality.

One fixed reset-inclusive nine-call ABBA gives A78.172/B94.750/B94.780/A78.258 us: candidate slowdown16.550 us per nine calls, versus A/B drift0.086/0.030 us. It stops without another tile or deployment timing. An initial host-stub namespace build failure was fixed before any numerical/timing data; no arithmetic or tile changed. The result does not isolate a hardware cause. Source, build failure/fix, numerical data and all120 timing samples remain in lab/pi05/softmax_pv.md and results/rtx5090-pi05/gpt6-attention-softmax-pv-screen. Production expert attention stays unchanged.
...
## Stopped single-launch bucket selection

CPU inspection found that one max-grid launch could select either original Stream-K parameter block and guard raw blockIdx.x against that bucket's original grid before entering the kernel. Existing binary metadata gives480-byte original Params and496-byte current BucketParams; two original parameter blocks plus selector/grid fields would remain below1KiB. Capacity alone does not establish a performance benefit: selecting parameter addresses may add per-thread copies or registers to the current254-register kernel, and the larger common grid would still launch extra inactive CTAs.

The023 overview measures only82.850 us across all51 inactive launches, about0.27% of forward latency before subtracting selection costs added to active GEMMs. This is insufficient reason to implement the more complex route now. No native candidate or compilation was performed. The source-based boundary and uncertainty are preserved in lab/pi05/bucket_single_launch_feasibility.md.

## Static M896 prefix projections screen

The same fixed cfg0 M896 reuse was screened separately at QKV and output projection using actual023 inputs:18 QKV layers and17 output projections. Both sites used one predeclared ABBA and retained all120 samples. The QKV chain leaves full968 RMSNorm/RoPE and shared working addresses in place; its projected reset is outside timing for both routes. Its total gain is only0.001712 ms, with overlapping per-leg distributions, so the runtime QKV candidate stops. All compared Q/K/V prefixes and full x_norm are exact.

Output projection restores each full968 residual inside the timed chain for both routes. ABBA gives A0.912320/B0.735360/B0.735104/A0.910144 ms. The0.176000 ms mean gain exceeds A/B drift0.002176/0.000256 ms; conservative separation is0.174784 ms. All17 candidates pass the existing895/896-row shallow limits, with worst rel_rms0.00011054231 and all physical rows finite. This changes both GEMM implementation and M; the changed Stream-K workspace/schedule prevents attributing its gain only to row-tile reduction. Source, reset/cache conditions and every numerical/timing result are in lab/pi05/prefix_static_m896.md and results/rtx5090-pi05/gpt6-prefix-projections-m896/local.json.

## 024 — Runtime bucketed backbone output projection, retained

The existing explicit-mask graph mapping now includes backbone output projection. Its candidate reuses the exact023 native bucket plans, beta1 wrapper and serial workspace roles; the dense control has a Torch alias accepting the same unused mask argument. No CUDA source, native build, numerical tolerance or extra workspace role is added. Physical prefix rows and KV offset remain968. Only this call site's backend changes in the loaded20-site plans.

A single-engine short896→long903→short896 official comparison passes, preserving graph identity, exact tokens and finite padded KV/actions. Short action cosine is0.9999944451, rel_rms0.0033334524 and max_abs0.01601866. The long case retains the prior action metrics; both short results agree. These are numerical error measurements, not policy-quality improvements. The11 scoped CPU tests and8 declarations plus1 route check also pass.

Four consecutive fresh-process deployment commands retain the existing seed42/5-warmup/100-sample protocol. ABBA medians are A30.306605/B30.185328/B30.194630/A30.378585 ms. Mean gain0.152616 ms exceeds A/B drift0.071980/0.009302 ms; minimum median separation0.111975 ms also exceeds the larger drift. Both candidate medians stay below both controls, so the route is retained without another block. Ending SM clocks are2865/2857/2857/2857MHz, memory13801MHz and temperatures56/58/59/60C. These ending observations do not measure full clock histories, and no causal frequency adjustment is applied.

Source3db1e73 implements the optional route and cad1246 selects it. The curve uses the first candidate median30.185328 ms; candidate medians span30.185328–30.194630 ms. All400 samples, complete route maps, official switch results and the decision remain under024 reports. The measured improvement still applies to the fixed895-valid-row workload; long-prefix correctness passes while its performance remains unmeasured. The023 overview and unchanged native resource evidence are reused rather than recaptured.

## Rejected M32 full-row backbone attention fusion

A fixed Triton M32/N1024 candidate computes the complete BF16-rounded QK row, FP32 scale/mask/softmax, BF16 probabilities and PV in one kernel. It retains full-row normalization,8 warps, one stage, QK BK32 and PV BK64. Actual compiled resources are255 registers,67,584 shared bytes and104 bytes of local storage per thread. Triton's n_spills=26 is the CUDA local-storage attribute divided by4, not a count of spill instructions. PTX checks confirm both BF16 boundaries and separate FP32 scale/add. Resource counts alone were not used to declare a slowdown.

A fresh capture from accepted024 supplies17 actual layers at895 valid prefix rows, using the same working Q/out addresses and identical Q resets inside both timed chains. All control outputs are bitwise exact to the capture; all candidate outputs pass existing shallow tolerances, with worst rel_rms0.000052171, minimum cosine0.999999998639 and max_abs0.03125. Both output buffers remain physically7744x256.

One predeclared ABBA,30 samples per leg, gives A1.158412/B1.752528/B1.754700/A1.190448 ms per17 calls. The candidate is slower by0.579184 ms, versus A/B drift0.032036/0.002172 ms; even the nearest medians are separated by0.562080 ms. This fixed mapping is rejected without deployment timing. The result does not isolate local-storage traffic, repeated K/V reads or gather/layout work as the cause. lab/pi05/backbone_fullrow.md and results/rtx5090-pi05/gpt6-backbone-fullrow retain the resource definitions, BF16 evidence, actual numerical checks and all120 samples.

A separate follow-up was declared only after that result: BM16 with the same8 warps/stage/K tiles/gather, to test reduced register pressure against doubled logical K/V reads. It reuses the024 snapshot and gives every new result an M16 suffix; original M32 data stays intact. This is one explicit variable change, without a tile or warp sweep.

The sole BM16 follow-up also stops. It compiles at206 registers,66,560 shared bytes and zero local storage, with both BF16 boundaries verified in PTX. All17 real layers pass the same shallow checks (worst rel_rms0.000052171). Its own ABBA gives A1.159620/B1.858880/B1.859224/A1.190400 ms per17 calls: candidate slowdown0.684042 ms, A/B drift0.030780/0.000344 ms and minimum separation0.668480 ms. All120 samples are retained with the M16 suffix.

Removing local storage was insufficient to make this fixed full-row algorithm competitive in the measured chain. M16 and M32 used separate ABBA blocks, so their two candidate means are not treated as a synchronized causal comparison. No BM8, warp/stage sweep, gather rewrite or production integration followed. The source supports the explicitly tested16/32 settings, and each implementation and result remains reviewable in commit history and results/rtx5090-pi05/gpt6-backbone-fullrow.
