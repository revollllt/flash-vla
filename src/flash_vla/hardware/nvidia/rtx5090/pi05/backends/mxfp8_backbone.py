...
not read. The mask is not used: all 968 rows are computed.
"""
...
class GemmPlan:
    """output [M, N] bf16 = activation [M, K] * weight [N, K]^T + beta * output,
    both operands MXFP8, bound to fixed addresses and one tile configuration."""
    ...
    def __init__(self, config: int, activation: ops.Activation, weight: ops.Activation,
        ...
        rows, depth = activation.rows, activation.cols
        cols = weight.rows
        if (weight.cols != depth or tuple(output.shape) != (rows, cols)
                or output.stride() != (cols, 1) or output.dtype != torch.bfloat16):
            raise ValueError(f"GEMM [{rows}, {depth}] x [{weight.rows}, {weight.cols}]^T cannot "
                             f"write {output.dtype} {tuple(output.shape)} {output.stride()}")
        kernels = library()
