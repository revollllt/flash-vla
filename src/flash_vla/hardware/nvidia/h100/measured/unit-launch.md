...
**Constants** `launch.lat.dev.ramp`, `ld.bw.dev.dram`, `ld.ctas.dev.knee`, `cluster.lat.sync`,
...
numbers and that lesson were both captured.
...
**Every kernel starts ~1.24 µs in debt** (`launch.lat.dev.ramp`), at 128 CTAs and above.
...
**`bytes / peak_BW` is not a floor** (`ld.bw.dev.dram`). A cold read costs
...
**CTA count still matters past machine coverage** (`ld.ctas.dev.knee`). At 4.19 MB:
...
**Declaring a cluster is free; synchronising one is not** (`cluster.lat.sync`).
...
(`cluster.count.max`). At 1 CTA/SM this machine places 66 clusters of 2, 30 of 4,
...
**Occupancy capacity is not resident warps** (`sched.ctas.sm.knee`). 128 CTAs on 132
...
- **The grid barrier is priced in `unit-coop.md`**, not here:
  `coop.lat.dev.sync` is 1.09 µs against a 1.40 µs device-side relaunch, so a
  grid barrier and a launch are comparable.
