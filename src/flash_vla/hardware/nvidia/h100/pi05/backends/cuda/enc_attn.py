"""Host side of the fused encoder (prefix) attention kernel.

Mirrors `attn_taskloop.py`: `build()` compiles `kernels/enc_attn.cu` into a
plain shared library under the repo's `.cache` (shared filesystem, so a
login-node build is visible to compute nodes) and loads it via ctypes; the
...
def _extra_flags() -> list[str]:
    """ENC_ATTN_NVCC_DEFINES: space-separated extra nvcc flags (kernel variants)."""
    return os.environ.get("ENC_ATTN_NVCC_DEFINES", "").split()
...
def _build_dir() -> Path:
    ...
    tag = hashlib.sha256(_SRC.read_bytes() + tile_headers
                         + " ".join(_extra_flags()).encode()).hexdigest()[:16]
    d = _REPO / ".cache" / "cuda_ext" / f"enc_attn_{tag}"
