...
class Pi0(VLA):
    ...
    #: two backbone call sites the shared Gemma component implements faster --
    #: the prefix attention as one fused MQA kernel instead of a four-launch
    #: torch chain that also copied its result, and the output projection on
    #: cuBLAS. The FFN down projection stays on TileLang: cuBLAS measured
    #: slower there in the graph, where the hidden buffer is L2-resident.
    plan = {
