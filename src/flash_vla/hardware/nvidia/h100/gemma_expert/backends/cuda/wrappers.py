"""Hand-written CUDA call sites of the Gemma action expert: attention and FFN.

Five call sites carrying the standard vocabulary's signatures, so a Target can
move its expert one piece at a time:

    {"action_expert_norm_qkv_rope": ..., "action_expert_attention": ...}
    {"action_expert_norm_gated_ffn": ..., "action_expert_ffn_down_residual": ...}

with "action_expert_out_proj_residual" joining the last pair on the fused
three-call route.
...
(`taskloop.FFNTaskloop`). `action_expert_norm_gated_ffn` launches the K-major
XFS producer, which resets the readiness counters itself, with a
programmatic-dependent-launch trigger; `action_expert_ffn_down_residual`
immediately launches the persistent consumer with wait mode 1 (every warp waits
at entry) or, on the `pdl_chain` variant, mode 2 (weight loaders released
early). The two calls are adjacent in the pipeline, which is the
direct-predecessor contract PDL requires. The Phase-2 route starts one call
earlier and treats out-projection, norm/gated-FFN, and down-residual as an
atomic three-call transaction.

Both halves run on the padded allocations behind a Target's views: 64 query
rows and a key extent padded to a multiple of the attention split's stage
span. Every wrapper recovers the padded base from the view it is handed and
refuses a view not cut from one.
...
`action_expert_attention` here requires `action_expert_norm_qkv_rope` here as
well: the route constraint rejects the split when the plan is resolved and the
wrapper checks again at call time.

Weight packing is cached during warmup and is absent from CUDA graph replay.
The geometry is compiled in: `kernels/sm90_attn_task_desc.cuh` fixes the row
count and the prefix length of the profile this build serves.
"""
...
from ..tilelang import producers as _producers
...
#: The two attention call sites are one unit: the route constraint refuses to
#: split them.
...
def make_wrappers(
    ...
    def action_expert_norm_qkv_rope(x, scale, weight_qkv, bias, rope, Q, K, V,
        ...
        _producers.rms_factor(x, norm_factor[:M],
    ...
    def action_expert_norm_gated_ffn(
        ...
        if fuse_out_proj:
            ...
            _producers.out_proj_residual_rms_xfs(
