...
| 8 | decoder attention split latency (7.65 us for ~1 MB) | decoder | ~0.5 ms | latency-bound; part of task-graph continuity | PRICED AND CLOSED (lane D1, no job spent): worth zero to weight-stream continuity -- the PDL chain already pre-issues the mask and every pure-prefix K/V frame above its wait, and the attention task loop already deals attention onto the CTAs idle during qkv, so the 5.4 us above its delivery ceiling is the Q-on-slowest-qkv-tile dependency, the split publish and the combine join. Note `.agents/notes/rejected/architecture/2026-09-07-expert-megakernel-pricing.md` |
