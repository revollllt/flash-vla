...
//
// WHAT THE UPSTREAM CARRIES THAT WAS WORTH KEEPING -- the flag barrier (one
// `atom.add.release.gpu` per CTA and one `ld.acquire.gpu` spin, reset by the
// last arriver; grid = SM count at 1 CTA/SM by __launch_bounds__, and the
// harness asserts grid <= SMs x occupancy because a larger grid deadlocks
// silently), redundant RMSNorm in every CTA instead of a hop, cache-policy
// weight loads, and bf16x2 unpack by shift/mask -- and the sm90 spellings
// this port had to learn (`L2::evict_first` is not legal on sm_90a, the
// policy goes through `createpolicy` and `L2::cache_hint`; `relaxed.cta` is
// a strong load, not a cache policy; `fma.rn.f32x2` and 256-bit loads are
// Blackwell-only) are on [kernel-megakernel-forms] and
