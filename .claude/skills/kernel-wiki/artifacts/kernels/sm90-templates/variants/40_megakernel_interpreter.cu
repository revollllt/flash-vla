...
//     instruction is slow.
//
// WHAT A MEGAKERNEL DOES NOT REMOVE, THE REGISTER TAX, AND WHY THIS CANNOT
// DEADLOCK are rules that hold beyond this file: [kernel-megakernel-forms,
// pattern-fusion-latency-chain, pattern-one-sided-gradient].  What they mean
// here: `y` and `h` below are gmem scratch exactly as they would be between
// three kernels; consumer warps are sized for the MAX over every op the VM
// can run; and pages belong to the ring stage, which is the obviously-correct
// version of the page-release argument.
//
...
// teaches the wrong thing: this is the ledger of [pattern-fusion-latency-chain] coming out
