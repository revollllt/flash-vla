...
//
// Norm, activation, rope, quantization and softmax are bound by bytes (row
// read plus row written: vector width, traversal count, fusion depth) and by
// launches ([launch.lat.dev.ramp] at decode shapes), which is why every one
// of them carries griddepcontrol: [technique-row-traversal-fusion].  Method
// follows FlashInfer (Apache-2.0) `include/flashinfer/{norm,activation,
