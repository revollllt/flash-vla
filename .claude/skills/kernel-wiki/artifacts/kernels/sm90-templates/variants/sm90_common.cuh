...
// the compiler barriers NVIDIA intends, and the two differ ASYMMETRICALLY: the
// wait is an acquire and must stop the compiler hoisting a producer-data load
// above it; the trigger deliberately is NOT a barrier, because overlapping the
// independent work around it is the entire point.  What each one promises,
// where each belongs (the wait as late as the data dependency allows, the
// trigger as early as measurement says) and why no fence precedes the trigger:
// [hw-pdl-gdc, technique-pdl-placement].
//
