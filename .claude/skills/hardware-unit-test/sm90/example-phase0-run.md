# The original Phase 0 calibration run — H100 SXM5, 2026-08-20

**Where this came from.** This was `tile-dataflow/references/example-phase0.md`
until Phase 0 moved into this skill; it is the run that produced the `launch`
unit's constants (`LAUNCH-RAMP`, `BW-CEIL`, `BW-CTA`, `CLUSTER-SYNC`,
`CLUSTER-PLACE`, `OCC-NOT-WARPS`) and it is kept here as their provenance —
those constants have no Slurm job ids, only this record.

Read `sm90/constants.yaml` for the current values; this file is the
evidence and the **method**: what a calibration run looks like, the 0a/0b split,
and why comparing against a library implementation is not a bound.

**Toolchain.** torch 2.11.0+cu130, CUDA 13.0 — five days and one torch minor
before the `tma` unit. Do not combine a number from here with one from there
without saying so.
