...
//
//  1. STATIC PER-SM INSTRUCTION STREAMS. instructions[worker][i][32]: the
//     planner decided who runs what, in which order, before launch. No claim
//     cursor, no atomic per dispatch. Upstream indexes by %smid; blockIdx.x
//     is used here (grid == SM count, 1 CTA/SM, same thing without the trap).
//  2. PAGES CROSS INSTRUCTION BOUNDARIES INDIVIDUALLY. Each op declares the
//     ORDER in which it releases its 13 logical pages (`release_lid`); the
//     controller maps the next instruction's logical page i to the physical
//     page the current one releases i-th. A per-page semaphore (`page_finished`,
//     16 arrivals = one per consumer warp) is what the next instruction's
//     loader waits on -- so its weight stream starts into pages the previous
//     op has already drained, while that op is still computing on the rest.
//     The invariant that makes it sound: EVERY instruction releases EVERY
//     page exactly once (unused pages are released by the loader on entry).
//  3. WEIGHTS ARE PREFETCHED ACROSS THE DEPENDENCY. The loader never waits
//     on a global counter. Only the consumer's read of the ACTIVATION waits
//     (`gmem_wait`), by which time up to three stages of weights are already
//     in shared memory. This is the whole reason the machine has no bubbles:
//     the dependency hop [atom.lat.dev.hop] is paid under the weight stream,
//     not in front of it. Template 40 waited before loading anything, and
//     lost.
//  4. GLOBAL-MEMORY COUNTERS, ONE PER (layer, op, slice). Bar[layer][op][k]
//     is bumped by the storer after its async-proxy stores complete; the
//     consumer that needs the data spins on it. Down-proj waits per
//     reduction-column block, so it starts on the first quarter of up/gate
//     while the rest is still running; attention's last KV block waits for
//     THIS layer's K/V append while every earlier block streams in
//     beforehand.
//  5. THE PLANNER. Blocks of 16 output rows are the unit; instructions carry
//     lists of them; the per-SM lists come from round-robin over a
//     topologically ordered DAG (upstream default `rr`) or from
//     cost-weighted list scheduling (`dag`). A topological order per SM plus
//     counters that only ever count up is what makes the static schedule
//     deadlock-free.
//
