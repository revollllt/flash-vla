...
//
// FOUR MECHANISMS THAT ARE THE POINT
//
//  1. CUMULATIVE EVENT COUNTERS. Nothing is ever reset between decode steps:
//     iteration i waits for num_triggers x i. That is what lets the kernel
//     run token after token without a grid-wide barrier or a relaunch.
//  2. TWO LAUNCH MODES OF THE SAME GRAPH. `aot` (upstream's current default):
//     every LAUNCH event is demoted to EMPTY, every task gets it as its
//     dependent_event, and one LAUNCH_DEPENDENT_TASKS event pre-enqueues the
//     whole graph in topological order, round robin over workers -- the
//     schedulers then only handle the end of each step. `jit`: events really
//     launch their dependents when they fire, so a task lands in a queue only
//     once it can run. Both are here; the harness runs both.
//  3. EVENT GRANULARITY IS A COMPILER DECISION. An edge between two ops is cut
//     into `event_dim` events, each triggered by a contiguous producer range
//     and launching a contiguous consumer range. Here qkv -> attention is cut
//     per KV head (8 events, 24 triggers each) and attention -> merge likewise;
//     that needs the QKV weight rows laid out so a KV head's q, k and v rows
//     are contiguous, which is the kind of layout choice the compiler makes so
//     partitions line up (upstream: GCD/LCM unification).
//  4. CROSS-TASK PRE-LOADING. A task's weights have no dependency, so the
//     worker puts the NEXT task's weight range into flight (bulk L2 prefetch)
//     before spinning on the CURRENT task's event. Upstream describes this as
//     overlapping T2's pre-loading phase with T1's compute and measures it at
//     1.2-1.3x on its linear tasks; the harness toggles it (prefetch=0).
//
