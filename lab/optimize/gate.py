...
The promotion gate: one candidate plan against the acceptance registry, one evidence record.
    ...
    python -m lab.optimize.gate --target h100/pi05
    python -m lab.optimize.gate --target h100/pi05 --candidate lab/plans/pi05-attn-cuda.json
    python -m lab.optimize.gate --target h100/pi05 --mode no-regression --baseline

A script, not a service. It reads the Target's entry of the acceptance
registry (`lab/optimize/policy.py`), runs the in-engine correctness checks and the
...
from tools.profiling import floor as floor_model
from benchmarks import latency
from flash_vla.inference import declare, resolve
...
def _registry_version() -> str | None:
    ...
    if shutil.which("git") is None:
        return None
    return subprocess.run(["git", "log", "-1", "--format=%H", "--", "lab/optimize/policy.py"],
