...
Reproduce this lab probe in an isolated checkout at `f46185a`, which includes
its shared-memory resource-query ABI. The later production implementation
intentionally omits that lab-only ABI; the probe is not a production command.
