"""Rejected fixed CUTLASS M16 FFN-down screen.

Apply cutlass_m16_down.patch first; control and candidate must share the one
updated native library. See cutlass_m16_down.md for the bounded reproduction.
"""
...
class _Plan:
    def __init__(self, library, scratch, role, x, weight, gate, out):
        size = library.expert_m16_down_workspace()
        self.workspace = scratch(role, (size,), torch.uint8, x.device)
        self.tensors = (x, weight, gate, out)
        self.handle = ctypes.c_void_p()
        cutlass_backbone._check(library.expert_m16_down_plan(
            x.data_ptr(), weight.data_ptr(), gate.data_ptr(), out.data_ptr(),
            self.workspace.data_ptr(), torch.cuda.current_stream().cuda_stream,
            ctypes.byref(self.handle)), "expert_m16_down_plan")
        self.destroy = weakref.finalize(self, library.expert_m16_down_destroy, self.handle)


def make_candidate(library, scratch):
    library.expert_m16_down_workspace.argtypes = []
    library.expert_m16_down_workspace.restype = ctypes.c_int64
    library.expert_m16_down_plan.argtypes = (
        [ctypes.c_void_p] * 6 + [ctypes.POINTER(ctypes.c_void_p)])
    library.expert_m16_down_plan.restype = ctypes.c_int32
    library.expert_m16_down_run.argtypes = [ctypes.c_void_p] * 2
    library.expert_m16_down_run.restype = ctypes.c_int32
    library.expert_m16_down_destroy.argtypes = [ctypes.c_void_p]
    library.expert_m16_down_destroy.restype = None
    plans = {}
    role = f"lab_m16_down_streamk_{id(plans)}"
    ...
    def invoke(x, weight, gate, out):
        ...
        if plan is None:
            ...
            plans[key] = plan
        cutlass_backbone._check(library.expert_m16_down_run(
            plan.handle, torch.cuda.current_stream().cuda_stream), "expert_m16_down_run")
        return out
    return invoke
...
def main():
    ...
    report = {
        ...
        "shape_mkn": [50, 4096, 1024], "tile": [16, 64, 32],
        "warp": [16, 32, 32], "stages": 8,
    ...
    try:
        ...
        report.update(native_library=library._name,
                      ...
                      candidate_workspace_bytes=library.expert_m16_down_workspace())
        if args.phase == "coverage":
            ...
            for name, input_value, gate_value, residual in (
                ...
                if not row["exact"]:
                    raise RuntimeError(f"M16 down constant coverage failed: {name}")
        ...
        for index, call in enumerate(calls):
            ...
            save()
            if not valid:
                raise RuntimeError(f"M16 down shallow check failed at call {index}")
        ...
        for index in (0, len(calls) // 2, len(calls) - 1):
            ...
            save()
            if not row["exact"]:
                raise RuntimeError(f"M16 down rounding decomposition failed at call {index}")
        scratch.freeze()
