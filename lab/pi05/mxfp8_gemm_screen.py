...
CONFIGS = {0: "128x128x128 persistent", 1: "128x64x128 persistent", 2: "128x32x128 persistent",
           3: "128x128x128 stream-K", 4: "128x64x128 stream-K", 5: "128x128x128 split-K 2",
           6: "128x128x128 split-K 3", 7: "128x128x128 split-K 4",
           8: "128x128x128 pingpong"}   # mxfp8_backbone.cu
        ...
        for rows_used in BUCKET_ROWS:
            ...
            for config, tiles in CONFIGS.items():
                ...
                row = dict(gemm=gemm, m=rows_used, n=cols, k=depth, beta=beta, config=config,
                           ...
                           tflops=2 * rows_used * cols * depth / median_us / 1e6,
                           rel_rms_against_config0=float(
                               (layer_output - first_layer).norm() / first_layer.norm()),
                           samples_us=samples_us)
                rows.append(row)
                print(f"{gemm:8s} M={rows_used} config {config} {tiles:24s} {median_us:8.1f} us "
