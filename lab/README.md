...
| `sbatch/` | the Slurm wrappers of those scripts; submit from the repository root (`sbatch lab/sbatch/<name>.sh`); `campaign_gate.sh` gates each Target's shipped plan against its pre-campaign plan (`plans/<target>-preshipped.json`) and re-takes the floor report |
