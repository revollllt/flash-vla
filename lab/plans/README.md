...
| `lingbot_vla-skinny-projections.json` | plus the expert's two 768-wide output projections on the hand-written weight-stationary GEMM, whose split-K partials reduce through distributed shared memory. The packed q/k/v and gate/up projections are deliberately NOT here: the same kernel measured 5.12 and 5.91 us against cuBLAS's 4.79 and 5.84 on those shapes |
| `lingbot_vla-split-attention.json` | plus the expert's attention on the split-key CUDA kernel, two launches instead of the four-launch cuBLAS chain. This is what the LingBot Target ships |
